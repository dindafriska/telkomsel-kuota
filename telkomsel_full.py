#!/usr/bin/env python3
"""Telkomsel full auth + quota - single execution"""
import urllib.request, urllib.parse, json, ssl, hashlib, base64, secrets, sys, os
...
def gen(at,idt):
    ...
    a=en('{"accessToken":"'+at+'","timestamp":"'+ts+'"}')
    u=en('{"token":"'+idt+'","timestamp":"'+ts+'"}')
    return 'Bearer '+a,'Bearer '+u
...
print('  OK - OTP accepted')

print('Step 3: Auth code...')
...
qh={'AccessAuth':aa,'Authorization':au,'X-Device':xd,'sah':rh(28),'Web-App-Version':'2.0.0','Accept-Language':'id-ID,id;q=0.9','User-Agent':BASE['User-Agent']}
    ...
    if gs:
        for g in gs:
            ...
            for i in g.get('bonusList',[]):
                ...
                e=i.get('expirydate','?')
                print('  '+n)
                print('    Remaining: '+r+'  |  Exp: '+e)
    else:
        print('  No quota data')
else:
    print('  Error: '+bq.get('message','?'))
